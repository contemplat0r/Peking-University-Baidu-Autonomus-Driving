...
def str2coords(s, names=['id', 'yaw', 'pitch', 'roll', 'x', 'y', 'z']):
    ...
    Output:
        list of dicts with keys from `names`
    '''
    print("str2coords: s", s)
    coords = []
    for l in np.array(s.split()).reshape([-1, 7]):
        print("str2coords: l", l)
        coords.append(dict(zip(names, l.astype('float'))))
        print("str2coords: names: {}, l: {} ".format(names, l.astype('float')))
        if 'id' in coords[-1]:
            coords[-1]['id'] = int(coords[-1]['id'])
    return coords
...
# Распределение количества описываемых автомобилей в одном prediction string
...
def investigate_get_img_coords(s):
    '''
    ...
    print("\nimg_p[:, 2]", img_p[:, 2])
    img_p[:, 0] /= img_p[:, 2]
    print("img_p[:, 0] /= img_p[:, 2]", img_p[:, 0])
    img_p[:, 1] /= img_p[:, 2]
    print("img_p[:, 1] /= img_p[:, 2]", img_p[:, 1])
    img_xs = img_p[:, 0]
    print("img_xs", img_xs)
    img_ys = img_p[:, 1]
    print("img_ys", img_ys)
...
investigate_get_img_coords(train['PredictionString'][2217])
...
def visualize(img, coords):
    ...
    for point in coords:
        ...
        t = np.array([x, y, z])
        print("\n", "*" * 10)
        print("\n  visualize,  before Rt[:3,3]=t, Rt:\n", Rt)
        print("\n  visualize,  before Rt[:3,3]=t, Rt[:3,3]:\n", Rt[:3, 3])
        Rt[:3, 3] = t
        print("\n  visualize, after Rt[:3,3]=t, Rt:\n", Rt)
        print("\n  visualize, after Rt[:3,3]=t, Rt[:3, 3]:\n", Rt[:3, 3])
        print("*" * 10, "\n")
        
        print("\n", "*" * 10)
        print("\n  visualize, before Rt[:3, :3] = euler_to_Rot(yaw, pitch, roll).T, Rt:\n", Rt)
        print("\n  visualize, before Rt[:3, :3] = euler_to_Rot(yaw, pitch, roll).T:\n", Rt[:3, :3])        
        Rt[:3, :3] = euler_to_Rot(yaw, pitch, roll).T
        print("\n  visualize, after Rt[:3, :3] = euler_to_Rot(yaw, pitch, roll).T, Rt:\n", Rt)
        print("\n  visualize, after Rt[:3, :3] = euler_to_Rot(yaw, pitch, roll).T:\n", Rt[:3, :3])
        print("*" * 10, "\n")
        
        print("\n", "*" * 10)
        print("\n  visualize,  before Rt[:3,:]=t, Rt:\n", Rt)
        print("\n  visualize,  before Rt[:3,:]=t, Rt[:3,:]:\n", Rt[:3, 3])        
        Rt = Rt[:3, :]
        print("\n  visualize,  after Rt[:3,:]=t, Rt:\n", Rt)
        print("\n  visualize,  after Rt[:3,:]=t, Rt[:3,:]:\n", Rt[:3, 3]) 
        print("*" * 10, "\n")
